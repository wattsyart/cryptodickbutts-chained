...
namespace CryptoDickbuttsChained.Shared
{
    public static class ParityService
    {
        ...
        public static async Task StartAsync(ParityOptions parityOptions, Web3Options web3Options, ParityState state, HttpClient http, ILogger? logger, CancellationToken cancellationToken)
        {
            ...
            {
                throw new InvalidOperationException("Must provide CryptoDickbuttsChained Contract Address");
            }

            if (string.IsNullOrWhiteSpace(web3Options.OnChainRpcUrl))
            {
                throw new InvalidOperationException("Must provide CryptoDickbuttsChained RPC URL (Target)");
            }
            ...
            foreach (var tokenId in parityOptions.Scope.TokenIds())
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(sourceMetadata.Image))
                    {
                        ...
                        var imageUri = $"{DataUri.Png}{encoded}";
                    ...
                    var targetTokenUri = await DickbuttsService.GetCanonicalTokenURIAsync(tokenId,
                ...
                {
                    logger?.LogError("Unknown error while fetching CryptoDickbutt #{TokenId}: {Error}", tokenId, e);
